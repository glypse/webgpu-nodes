...
export const remapNode = {
	...
	defaultData: { e: 0, in_min: 0, in_max: 1, out_min: 0, out_max: 1 },
	wgsl: ({ varName, inputs }) => {
		const e = inputs.e[0];
		const in_min = inputs.in_min[0];
		const in_max = inputs.in_max[0];
		const out_min = inputs.out_min[0];
		const out_max = inputs.out_max[0];
		return [
